...
bbscore.py
python bbscore_2.py \
  --test-json /home/spandan/scratch/interiit/GAURAV_BIG_DATA/SAR_BIG/pair_data/train/final_all.json \
  --image-dir /home/spandan/scratch/interiit/GAURAV_BIG_DATA/SAR_BIG/pair_data/train/sar/img \
  --checkpoint /home/spandan/scratch/interiit/qwen/checkpoints_tim2/checkpoint-195 \
  --batch-size 4 \
  --max-samples 100 \
  --output-file /home/spandan/scratch/interiit/results_compare_tim.json
...
class QwenBatchDataset(Dataset):
    ...
    def _resolve_image_path(self, img_field):
        # if absolute path already, use it
        if os.path.isabs(img_field) and os.path.exists(img_field):
            return img_field
        # join with image_dir
        candidate = os.path.join(self.image_dir, img_field)
        if os.path.exists(candidate):
            return candidate
        # try basename search
        basename = os.path.basename(img_field)
        candidate2 = os.path.join(self.image_dir, basename)
        if os.path.exists(candidate2):
            return candidate2
        # fallback: look for file anywhere under image_dir
        name_no_ext = os.path.splitext(basename)[0]
        for root, _, files in os.walk(self.image_dir):
            for f in files:
                if os.path.splitext(f)[0] == name_no_ext:
                    return os.path.join(root, f)
        raise FileNotFoundError(f"Image not found: {img_field} (tried {candidate}, {candidate2})")

    def __getitem__(self, idx):
        entry = self.entries[idx]

        # image field in your JSON: "image": "multires-sar_Low-Resolution_502.png"
        img_field = entry.get("image") or entry.get("file_name") or entry.get("image_id")
        if img_field is None:
            raise ValueError(f"No image field in entry: {entry.get('id','<no-id>')}")
        img_path = self._resolve_image_path(img_field)
        image = Image.open(img_path).convert("RGB")

        # conversations list: extract question (human) and reference (gpt)
        convs = entry.get("conversations", []) or []
        question = None
        reference = None
        # prefer last occurrences
        for msg in reversed(convs):
            role = msg.get("from", "").lower()
            val = msg.get("value", "")
            if role == "gpt" and val and reference is None:
                reference = val
            if role == "human" and val and question is None:
                # strip <image> tags
                question = val.replace("<image>\n", "").replace("<image>", "").strip()
            if question and reference:
                break

        # defaults if missing
        if question is None:
            question = entry.get("question", "Please describe the image in detail.")
        if reference is None:
            # try other fallback fields
            reference = entry.get("ground_truth") or entry.get("caption") or ""
...
def load_finetuned_model(checkpoint_path: str, device="cuda"):
    # base snapshot you used for training
    base_model_name = "/home/spandan/scratch/interiit/qwen/models--unsloth--Qwen3-VL-8B-Instruct/snapshots/11d38e30f7b6dec7545b704d119dc6fbecbbd639"
    ...
    # pass the directory that contains adapter_model.safetensors & adapter_config.json
    model = PeftModel.from_pretrained(model, checkpoint_path, device_map="auto")
    # merge improves inference performance and removes PEFT overhead
