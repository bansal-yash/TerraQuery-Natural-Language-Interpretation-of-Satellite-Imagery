...
class CustomTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        print("Badi gaali: ", inputs["pixel_values"].shape)
        # sys.exit()
        ...
        with torch.no_grad():
            for i, sample in enumerate(self.eval_samples[:2]):  # Evaluate 5 samples
                try:
                    ...
                    print("gaali: ", inputs["pixel_values"].shape)
...
class CaptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Caption field handling: 'caption', 'captions', or 'answer'
        caption = entry.get('caption') or entry.get('captions') or entry.get('answer')
        if isinstance(caption, list):
            # choose first caption if list
        ...
        # print(f"\n[Sample {idx}] Masking verification:")
        # print(f"  Total tokens: {labels[0].shape[0]}")
        # print(f"  Masked (prompt): {masked_count} tokens ({100*masked_count/labels[0].shape[0]:.1f}%)")
        # print(f"  Training (caption): {unmasked_count} tokens ({100*unmasked_count/labels[0].shape[0]:.1f}%)")
        
        # # Decode what we're training on
        # trainable_ids = inputs['input_ids'][0].clone()
        # trainable_ids[labels[0] == -100] = self.processor.tokenizer.pad_token_id
        # trainable_text = self.processor.decode(trainable_ids, skip_special_tokens=True)
        # print(f"  Training text: '{trainable_text.strip()}'")
        
        # if unmasked_count < 20:
        #     print(f"  ⚠️ WARNING: Very few tokens to learn from!")
...
def main():
    ...
    for i in range(min(5, len(train_dataset))):
        try:
            ...
            # Get caption
            caption = entry.get('caption') or entry.get('answer')
            if isinstance(caption, list):
                caption = caption[0]
...
# python finetune_qwen_caption_earthmind.py --train-json /home/samyak/scratch/interiit/EarthMind-Bench/json/caption_all_unmatched.json --local-model-dir --image-dir /home/samyak/scratch/interiit/EarthMind-Bench/img/test/sar/img --use-lora

# bbscore base and only_sars
# bbscore base and sars+ft
# 


# python finetune_qwen_caption_earthmind.py --train-json /home/samyak/scratch/interiit/EarthMind-Bench/json/caption_all_unmatched.json --model /home/samyak/scratch/interiit/qwen/benchmarking_checkpoints/rgb_new_para --image-dir /home/samyak/scratch/interiit/EarthMind-Bench/img/test/sar/img --use-lora 