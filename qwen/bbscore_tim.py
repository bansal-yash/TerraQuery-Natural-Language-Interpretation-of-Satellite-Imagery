...
python bbscore_tim.py \
  ...
  --batch-size 1 \
  --max-samples 100 \
  --output-file /home/spandan/scratch/interiit/qwen/results_compare_tim.json
"""
...
class QwenBatchDataset(Dataset):
    ...
    def _get_image_path(self, entry):
        ...
        if os.path.isabs(img_path):
            ...
            for ext in (".png", ".jpg", ".jpeg"):
                ...
                if os.path.exists(alt):
                    return alt
            raise FileNotFoundError(f"Absolute image path not found: {img_path}")

        # join with provided image_dir
        ...
        for ext in ("", ".png", ".jpg", ".jpeg"):
            candidate = os.path.join(self.image_dir, filename + ext) if ext else os.path.join(self.image_dir, filename)
            if os.path.exists(candidate):
                return candidate

        # try to find by filename anywhere under image_dir
        ...
        tried = [full_path, os.path.join(self.image_dir, filename)]
        raise FileNotFoundError(f"Image file not found for entry: {entry}. Tried: {tried}")


    def _sanitize_text(self, text: str) -> str:
        if not isinstance(text, str):
            return text
        # remove image token markers and surrounding whitespace
    ...
    def _get_reference(self, entry):
        ...
        if convs and isinstance(convs, list) and len(convs) > 0:
            ...
            for msg in reversed(convs):
                if msg.get("from", "").lower() == "gpt" and msg.get("value"):
                    return self._sanitize_text(msg["value"])
        ...
        if reference:
            return self._sanitize_text(reference)
