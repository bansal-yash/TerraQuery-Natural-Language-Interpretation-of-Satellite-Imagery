...
class Orchestrator:
    ...
    def get_feature_list(
        ...
        Args:
            ...
            custom_prompt: Custom prompt to send to the VLM. If None, uses a default
                           prompt asking for visible features/objects in the image.
        ...
        if custom_prompt is None:
            custom_prompt = (
                """List the TYPES of objects visible in the image as a comma-separated list on one line. Rules: 
• One word per type, lowercase, PLURAL form (e.g., "trees", "cars", "buildings") 
• No duplicates, no synonyms repeated, no counts, no extra text 
• Omit uncertain/ambiguous items
Output example: buildings,roads,trees,cars
Final answer:"""
            )
        ...
        try:
            # Use a strict system prompt for feature extraction, scoped to the user's query
            focus_prompt = (custom_prompt or "the user's question").strip()
            self.vlm.system_prompt = (
                f"""You are a strict extractor of object types from aerial images. List ONLY the visually detectable object types that are directly relevant to answering this request: \"{focus_prompt}\". Output a single line containing a comma-separated list of unique, lowercase, PLURAL nouns (one word each, e.g., "trees", "cars", "buildings"). Only include objects relevant to the user's request. Ignore unrelated objects, omit uncertain items, avoid duplicates/synonyms, and provide no explanations or extra punctuation beyond commas."""
            )
            self.vlm.stream_thoughts = False
            self.vlm._suppress_debug_prints = True
