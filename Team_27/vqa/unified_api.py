...
def _clamp_rect_to_bounds(
    ...
    """Clamp a rotated rectangle to strictly fit within image bounds.
    
    This ensures all 4 corners are within [0, img_width-1] x [0, img_height-1]
    while maintaining a proper rectangular shape.
    ...
    Returns:
        Four (x, y) tuples representing a valid rectangle within bounds.
    """
    import cv2
    import numpy as np
    ...
    # Check if all points are already within bounds
    min_x = min(p[0] for p in box)
    max_x = max(p[0] for p in box)
    min_y = min(p[1] for p in box)
    max_y = max(p[1] for p in box)
    
    if min_x >= 0 and max_x <= img_width - 1 and min_y >= 0 and max_y <= img_height - 1:
        # Already within bounds
        return [(float(x), float(y)) for x, y in box]
    
    # Calculate how much we need to shrink/shift to fit within bounds
    # First, clamp the center to be within a safe region
    half_diag = np.sqrt(w*w + h*h) / 2.0
    
    # Compute the actual extents of the rotated box from center
    dx_extent = (max_x - min_x) / 2.0
    dy_extent = (max_y - min_y) / 2.0
    
    # Clamp center so the box fits
    new_cx = max(dx_extent, min(img_width - 1 - dx_extent, cx))
    new_cy = max(dy_extent, min(img_height - 1 - dy_extent, cy))
    
    # If the box is too large to fit even when centered, we need to shrink it
    need_shrink = False
    if dx_extent * 2 > img_width - 1:
        need_shrink = True
    if dy_extent * 2 > img_height - 1:
        need_shrink = True
    
    if need_shrink:
        # Shrink the rectangle uniformly to fit within bounds
        scale_x = (img_width - 1) / (dx_extent * 2) if dx_extent > 0 else 1.0
        scale_y = (img_height - 1) / (dy_extent * 2) if dy_extent > 0 else 1.0
        scale = min(scale_x, scale_y, 1.0)
        
        new_w = w * scale
        new_h = h * scale
        new_cx = (img_width - 1) / 2.0
        new_cy = (img_height - 1) / 2.0
        
        new_rect = ((new_cx, new_cy), (new_w, new_h), angle)
        result_box = cv2.boxPoints(new_rect)
    else:
        # Just shift the center
        shift_x = new_cx - cx
        shift_y = new_cy - cy
        result_box = np.array([(p[0] + shift_x, p[1] + shift_y) for p in box])
    
    # Convert to list of tuples - the rectangle is guaranteed to be within bounds
    # after the shift/shrink operations above
    return [(float(x), float(y)) for x, y in result_box]
...
def _compute_oriented_box_points(
    ...
    Always returns a proper rectangle strictly within image bounds.
    """
    if not bbox_pixels or len(bbox_pixels) != 4:
        return []
    ...
    angle_deg = target_orientation - initial_orientation
    
    # Create the rect in cv2.minAreaRect format: ((cx, cy), (w, h), angle)
    rect = ((cx, cy), (width, height), -angle_deg)
    box = cv2.boxPoints(rect)
    
    # Check if all corners are within bounds
    out_of_bounds = any(
        x < 0 or x > img_width - 1 or y < 0 or y > img_height - 1
        for x, y in box
    )
    
    if out_of_bounds:
        # Create a canvas and draw the rectangle, then recompute minAreaRect
        # from the visible pixels
        mask_canvas = np.zeros((img_height, img_width), dtype=np.uint8)
        box_int = np.int32(box)
        cv2.fillPoly(mask_canvas, [box_int], 255)
        
        visible_points = cv2.findNonZero(mask_canvas)
        if visible_points is None or len(visible_points) < 3:
            # Fallback: clamp the original rect
            return _clamp_rect_to_bounds(rect, img_width, img_height)
        
        clipped_rect = cv2.minAreaRect(visible_points)
        # Ensure the clipped rect is also within bounds
        return _clamp_rect_to_bounds(clipped_rect, img_width, img_height)
...
def _compute_mask_box_points(
    ...
    # Ensure mask dimensions match expected image dimensions for consistency
    ...
    # Get the minimum area rectangle from the mask
    rect = cv2.minAreaRect(points)
    box = cv2.boxPoints(rect)
    
    # Check if all corners are within bounds
    out_of_bounds = any(
        x < 0 or x > mask_w - 1 or y < 0 or y > mask_h - 1
        for x, y in box
    )
    ...
    if out_of_bounds:
        # Create a canvas and draw the rectangle, then recompute minAreaRect
        # from the visible pixels
        mask_canvas = np.zeros((mask_h, mask_w), dtype=np.uint8)
        box_int = np.int32(box)
        cv2.fillPoly(mask_canvas, [box_int], 255)
        ...
        if visible_points is None or len(visible_points) < 3:
            # Fallback: clamp the original rect
            clipped_points = _clamp_rect_to_bounds(rect, mask_w, mask_h)
        else:
            clipped_rect = cv2.minAreaRect(visible_points)
            # Ensure the clipped rect is also strictly within bounds
            clipped_points = _clamp_rect_to_bounds(clipped_rect, mask_w, mask_h)
    else:
        clipped_points = [(float(x), float(y)) for x, y in box]
    
    # If mask dimensions differ from provided image dimensions, scale accordingly
    if mask_w != img_width or mask_h != img_height:
        scale_x = img_width / mask_w
        scale_y = img_height / mask_h
        clipped_points = [
            (x * scale_x, y * scale_y) for x, y in clipped_points
        ]
        # After scaling, clamp again to ensure within target bounds
        clipped_points = [
            (max(0.0, min(x, img_width - 1.0)), max(0.0, min(y, img_height - 1.0)))
